...
string Elem::GetType(const string& aEType) const
{
    string res;
...
string Elem::GetPType(const string& aEType) const
{
    string res;
    size_t pos = aEType.find_last_of(GUri::KParentSep);
...
TEhr Elem::ProcessCompChanged(MElem& aComp, const string& aContName)
{
...
TEhr Elem::ProcessChanged(MElem& aComp)
{
    Pdstat(PEvents::DurStat_OnCompChanged, true);
    TEhr res = EEHR_Ignored;
    TBool hres = EFalse;
...
TBool Elem::RmNode(const ChromoNode& aSpec, TBool aRunTime, TBool aCheckSafety, TBool aTrialMode, const MElem* aCtx)
{
	...
	*/
    ...
    void Elem::DumpIfCtx(const TICacheRCtx& aCtx) const
    {
	cout << "<< Dump of Ifaces request context: START >>" << endl;
	for (auto item: aCtx) {
	    MElem* eitem = item->GetObj(eitem);
